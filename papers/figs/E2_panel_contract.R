# Paper E2 adapter for manifest-derived four-panel summaries.
...
if (is.null(.layout_dir)) {
  stop("E2_panel_contract.R cannot locate paper_theme_safe.R beside it",
       call. = FALSE)
}
...
load_E2_panel_summary <- function(artifact, root = getwd()) {
  if (!requireNamespace("jsonlite", quietly = TRUE)) {
    stop("Paper E2 panel summaries require the jsonlite package", call. = FALSE)
  }
  path <- file.path(root, "papers", "figs", "summaries", paste0(artifact, ".json"))
  if (!file.exists(path)) {
    stop(sprintf("missing Paper E2 panel summary: %s", path), call. = FALSE)
  }
  summary <- jsonlite::fromJSON(path, simplifyVector = FALSE)
  if (!identical(summary$artifact, artifact)) {
    stop(sprintf("Paper E2 summary artifact mismatch: expected %s", artifact),
         call. = FALSE)
  }
  if (!identical(as.integer(summary$panel_count), 4L) ||
      !is.list(summary$panels) || length(summary$panels) != 4L) {
    stop("Paper E2 summary must contain exactly four panels", call. = FALSE)
  }
  ids <- vapply(summary$panels, function(panel) as.character(panel$id), character(1))
  if (!identical(ids, letters[1:4])) {
    stop("Paper E2 summary panel IDs must be a, b, c, d", call. = FALSE)
  }
  for (i in seq_along(summary$panels)) {
    ...
    if (length(question) != 1L || anyNA(question) || !nzchar(trimws(question)) ||
      ...
      stop(sprintf("Paper E2 summary panel %s lacks question/source metadata", ids[[i]]),
           call. = FALSE)
    }
  }
  summary
...
compose_E2_four_panel <- function(panels, artifact, width, height, root = getwd()) {
  summary <- load_E2_panel_summary(artifact, root)
  if (!is.list(panels) || length(panels) != 4L) {
    stop("compose_E2_four_panel requires exactly four plots", call. = FALSE)
  }

  config <- load_layout_config(artifact, root, prefix = 'E2')
